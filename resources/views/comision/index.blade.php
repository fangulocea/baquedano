...
<div id="tabla" class="col-sm-12">
    <div class="white-box">
        <h3 class="box-title m-b-0">Administración de Comisiones</h3>
        <p class="text-muted m-b-30">Gestionar Comisión</p>
                ...
                <tbody>
                    @foreach($comision as $com)
                            <tr>
                                ...
                                <td>{{ $com->nombre }}</td>
                                <td>{{ $com->comision }} %</td>
                                <td>{{ trans_choice('mensajes.vigencia', $com->estado ) }}</td>
                                @can('comision.show')
                                <td width="10px">
                                    <a href="{{ route('comision.show', $com->id) }}" 
                                ...
                                @can('comision.edit')
                                <td width="10px">
                                    <a href="{{ route('comision.edit', $com->id) }}"><span class="btn btn-warning btn-circle btn-lg"><i class="ti-pencil-alt"></i></span></a>
                                </td>
                                @endcan
                                @can('comision.destroy')
                                <td width="10px">

                                    {!! Form::open(['route' => ['comision.destroy', $com->id], 
                                    'method' => 'DELETE']) !!}
    ...
    buttons: [
        'copy', 'csv', 'excel', 'pdf', 'print',{
            ...
            action: function ( e, dt, node, config ) {
                 window.location.href = '{{ route("comision.create") }}';
            }
        }
