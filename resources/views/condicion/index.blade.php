...
<div id="tabla" class="col-sm-12">
    <div class="white-box">
        <h3 class="box-title m-b-0">Administración de Condiciones de servicios</h3>
        <p class="text-muted m-b-30">Gestionar Condiciones de servicios para un contrato</p>
                ...
                <tbody>
                    @foreach($cond as $condicion)
                            <tr>
                                ...
                                @can('condicion.show')
                                <td width="10px">
                                    <a href="{{ route('condicion.show', $condicion->id) }}" 
                                ...
                                @can('condicion.edit')
                                <td width="10px">
                                    <a href="{{ route('condicion.edit', $condicion->id) }}"><span class="btn btn-warning btn-circle btn-lg"><i class="ti-pencil-alt"></i></span></a>
                                </td>
                                @endcan
                                @can('condicion.destroy')
                                <td width="10px">

                                    {!! Form::open(['route' => ['condicion.destroy', $condicion->id], 
                                    'method' => 'DELETE']) !!}
...
$('#listusers').DataTable({
    ...
    buttons: [
        'copy', 'csv', 'excel', 'pdf', 'print',{
            text: 'Crear Condición',
            action: function ( e, dt, node, config ) {
                 window.location.href = '{{ route("condicion.create") }}';
            }
        }
