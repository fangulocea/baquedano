...
<div id="tabla" class="col-sm-12">
    <div class="white-box">
        <h3 class="box-title m-b-0">Administración de Flexibilidad</h3>
        <p class="text-muted m-b-30">Gestionar Flexibilidad</p>
        <div class="table-responsive">
            <table id="listusers" class="display nowrap" cellspacing="0" width="100%">
                ...
                <tbody>
                    @foreach($flex as $flexi)
                            <tr>
                                <td>{{ $flexi->id }}</td>
                                <td>{{ $flexi->nombre }}</td>
                                <td>{{ $flexi->descripcion }}</td>
                                <td>{{ trans_choice('mensajes.vigencia', $flexi->estado ) }}</td>
                               
                                <td width="10px">
                                     @can('flexibilidad.show')
                                    <a href="{{ route('flexibilidad.show', $flexi->id) }}" 
                                ...
                                <td width="10px">
                                    @can('flexibilidad.edit')
                                    <a href="{{ route('flexibilidad.edit', $flexi->id) }}"><span class="btn btn-warning btn-circle btn-lg"><i class="ti-pencil-alt"></i></span></a>
                                    @endcan
                                </td>
                                
                               
                                <td width="10px">
                                     @can('flexibilidad.destroy')
                                    {!! Form::open(['route' => ['flexibilidad.destroy', $flexi->id], 
                                    'method' => 'DELETE']) !!}
...
$('#listusers').DataTable({
    ...
    buttons: [
        'copy', 'csv', 'excel', 'pdf', 'print',{
            text: 'Crear Flexibilidad',
            action: function ( e, dt, node, config ) {
                 window.location.href = '{{ route("flexibilidad.create") }}';
            }
        }
